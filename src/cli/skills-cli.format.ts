...
export function formatSkillsCheck(report: SkillStatusReport, opts: SkillsCheckOptions): string {
  ...
  const notInjected = report.skills.filter(
  ...
  lines.push(`${theme.success("✓")} ${theme.muted("Model-visible:")} ${modelVisible.length}`);
  lines.push(`${theme.success("✓")} ${theme.muted("Slash-visible:")} ${commandVisible.length}`);
  ...
  if (agentId || agentFiltered.length > 0) {
    lines.push(`${theme.warn("🚫")} ${theme.muted("Filtered by agent:")} ${agentFiltered.length}`);
  }
  if (notInjected.length > 0) {
    lines.push(
      `${theme.warn("△")} ${theme.muted("Loaded but not injected:")} ${notInjected.length}`,
    );
  }
  ...
  if (modelVisible.length > 0) {
    lines.push("");
    lines.push(theme.heading("Ready and injected:"));
  ...
  if (notInjected.length > 0) {
    lines.push("");
    lines.push(theme.heading("Loaded but not injected:"));
    for (const skill of notInjected) {
      const emoji = normalizeSkillEmoji(skill.emoji);
      lines.push(
        `  ${emoji} ${sanitizeForLog(skill.name)} ${theme.muted("(disable-model-invocation=true)")}`,
      );
    }
  }
  ...
  if (agentFiltered.length > 0) {
    lines.push("");
    lines.push(theme.heading("Filtered by agent:"));
    for (const skill of agentFiltered) {
      const emoji = normalizeSkillEmoji(skill.emoji);
      lines.push(
        `  ${emoji} ${sanitizeForLog(skill.name)} ${theme.muted("(not in agent skills)")}`,
      );
    }
  }
